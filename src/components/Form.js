...
const schema = yup.object({
    ...
    telephone: yup.string().required("Phone required!").matches(/^(\+\d{2,3}\s)?\(?\d{3}\)?[\s.-]\d{2}[\s.-]\d{3}[\s.-]\d{4}$/, "Phone number must match the form 233 00 000 0000"),
    guests: yup.string().min(1, "There must be at least 1 guest!").required("Please specify number of guests per table!"),
    ...
    return (
        <form onSubmit={handleSubmit(formSubmit)}>
            <fieldset>
                ...
                <div className="field">
                    <label htmlFor="telephone">Phone<span className='required'>*</span></label>
